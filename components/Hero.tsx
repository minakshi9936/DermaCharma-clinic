...
export default function Hero() {
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="relative mx-auto grid max-w-7xl gap-10 px-4 pb-20 pt-32 text-white lg:grid-cols-[1.1fr_0.9fr]">
        <div className="space-y-8">
          <div className="space-y-4">
            <p className="text-xs font-semibold uppercase tracking-[0.4em] text-[#83C5BE]">{activeSlide.badge}</p>
            <h1 className="text-4xl font-bold leading-tight md:text-6xl">{activeSlide.heading}</h1>
            <p className="text-lg text-white/80 md:text-xl">{activeSlide.description}</p>
            <div className="flex flex-wrap gap-3">
              {activeSlide.highlights.map((item) => (
                <span key={item} className="rounded-full border border-white/30 bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-widest text-white/80">
                  {item}
                </span>
              ))}
            </div>
          ...
          <div className="flex flex-wrap items-center gap-6 text-sm text-white/70">
            <div>
              ...
              <p className="text-2xl font-semibold text-white">{activeSlide.focus}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-[0.4em] text-white/60">Result insight</p>
              <p className="text-lg font-medium text-white">{activeSlide.metric}</p>
            </div>
          </div>
          <div className="flex gap-2">
            {slides.map((slide, index) => (
              <button
                key={slide.id}
                type="button"
                className={`h-1 rounded-full transition-all ${index === activeIndex ? 'w-12 bg-[#83C5BE]' : 'w-6 bg-white/40'}`}
        ...
        <div className="rounded-3xl border border-white/10 bg-white/90 p-8 text-[#2A3D45] shadow-2xl shadow-[#0f1f24]/30 backdrop-blur">
          <p className="text-xs font-semibold uppercase tracking-[0.4em] text-[#83C5BE]">Quick enquiry</p>
          <h2 className="mt-3 text-3xl font-semibold">Reserve your personalised consult</h2>
          <p className="mt-2 text-sm text-[#2A3D45]/70">Share a few details and our concierge team will confirm your desired slot within one business day.</p>
          <form onSubmit={handleSubmit} className="mt-6 space-y-5">
            <div className="space-y-2">
              <label htmlFor="hero-name" className="text-sm font-medium text-[#2A3D45]">
                Full name
              </label>
              <input
                id="hero-name"
                ...
                className="w-full rounded-2xl border border-[#83C5BE]/40 bg-white px-4 py-3 text-sm text-[#2A3D45] outline-none transition focus:border-[#83C5BE] focus:ring-2 focus:ring-[#83C5BE]/40"
                placeholder="Enter your name"
              />
            </div>
            <div className="space-y-2">
              <label htmlFor="hero-contact" className="text-sm font-medium text-[#2A3D45]">
                Contact number
              </label>
              <input
                id="hero-contact"
            ...
            <div className="space-y-2">
              <label htmlFor="hero-message" className="text-sm font-medium text-[#2A3D45]">
                Message
              </label>
              <textarea
                id="hero-message"
            ...
            <button
              type="submit"
              className="w-full rounded-full bg-gradient-to-r from-[#2A3D45] via-[#83C5BE] to-[#E9F5F3] px-8 py-3 text-sm font-semibold uppercase tracking-[0.3em] text-white shadow-lg shadow-[#CDBBA7]/60 transition hover:-translate-y-0.5"
            >
              Request a call back
            </button>
          </form>
